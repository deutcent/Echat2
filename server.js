...
// === Enhanced File Upload - Now supports GIFs with better handling ===
...
// Enhanced file filter with size limits
const upload = multer({ 
  storage,
  limits: { 
...
// === Socket.IO Logic with Enhanced GIF Support ===
io.on('connection', socket => {
  ...
  // Image messages (including GIFs) - Enhanced handling
  socket.on('chat image', async (data) => {
    ...
    // Log GIF handling
    if (data.type === 'gif') {
      console.log('Processing GIF message:', data.filename);
    }
  ...
  // React to message - Enhanced to handle multiple reactions
...
server.listen(PORT, () => console.log(`✅ Enhanced Server with GIF support running at http://localhost:${PORT}`));